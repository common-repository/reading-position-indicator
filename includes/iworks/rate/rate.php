	...
	class iworks_rate {
		...
		public function add_donate_link( $actions, $plugin_file, $plugin_data, $context ) {
			...
			if ( ! empty( $settings_page_url ) ) {
				$actions['settings'] = sprintf(
					...
					esc_html__( 'Settings', 'reading-position-indicator' )
				);
			}
			$actions['donate'] = sprintf(
				...
				esc_html__( 'Provide us a coffee', 'reading-position-indicator' )
			);
			return $actions;
		}
		...
		private function get_plugin_data_by_plugin_id( $plugin_id ) {
			...
			$plugin['url'] = esc_url(
				...
				/* translators: %s: plugin slug */
					_x( 'https://wordpress.org/plugins/%s', 'plugins home', 'reading-position-indicator' ),
					$plugin['slug']
				)
			);
			$plugin['support_url'] = esc_url(
				sprintf(
					/* translators: %s: plugin slug */
					_x( 'https://wordpress.org/support/plugin/%s', 'plugins support home', 'reading-position-indicator' ),
					$plugin['slug']
				)
			);
			/**
		...
		public function filter_get_advertising_og( $data ) {
			return array(
				'iworks-adverting-og' => array(
					'title'    => __( 'OpenGraph', 'reading-position-indicator' ),
		...
		public function get_advertising_og_content() {
			$args = array(
				...
				'plugin_name'        => __( 'OG — Better Share on Social Media', 'reading-position-indicator' ),
				'plugin_wp_home'     => __( 'https://wordpress.org/plugins/og/', 'reading-position-indicator' ),
			);
